/* ══════════════════════════════════════════════════════════
   ...
   Alterado: 21-08-2026 as 00:19hs
   Alterado: 14-09-2026 (78.1) — processarReserva() não recebe mais os
   documentos em base64: reserva.html sobe os arquivos pro Drive ANTES
   (via painel/shared/apps-script-upload-documentos-agente.gs, ação
   crm_reserva_upload_documentos — projeto isolado numa conta pessoal,
   mesmo motivo do upload de documentos do agente) e manda só os links
   já prontos. Isso resolve o limite de anexo do Brevo (~13 MB por
   arquivo, e o e-mail inteiro podia estourar o limite da API com vários
   documentos) — agora o e-mail só lista links, então não tem mais
   tamanho que "sobra" e nenhum documento pode sumir silenciosamente:
   se algum arquivo falhar ao subir, data.documentosFaltando > 0 e o
   e-mail interno destaca isso.
   Arquivo canônico — este projeto só tinha backups .txt datados até
   14-09-2026; este .gs passa a ser a fonte de verdade daqui pra frente.
══════════════════════════════════════════════════════════ */
...
function processarReserva(data) {
  ...
  // 1. Documentos já chegam como LINKS do Drive (reserva.html subiu os
  // arquivos antes de mandar pra cá — ver comentário no topo do arquivo,
  // 78.1). Nada de base64/anexo aqui: sem limite de tamanho e sem
  // documento que "some" silenciosamente do e-mail.
  ...
  var documentosFaltando = parseInt(data.documentosFaltando, 10) || 0;
  ...
  if (temConjuge && docsConjuge.length > 0) {
    secaoDocsConjuge =
      ...
      '<ul style="font-size:14px;color:#333">' +
        docsConjuge.map(function (d) { return '<li>' + d.descricao + ': <a href="' + d.url + '" style="color:#087bb5">' + d.nome + '</a></li>'; }).join('') +
      '</ul>';
  }

  var secaoAlertaDocs = '';
  if (documentosFaltando > 0) {
    secaoAlertaDocs =
      '<div style="background:#FEF2F2;border-left:4px solid #DC2626;padding:14px 18px;margin-top:20px">' +
        '<p style="color:#DC2626;margin:0;font-size:14px;font-weight:bold">⚠️ ' + documentosFaltando + ' documento(s) NÃO foram recebidos.</p>' +
        '<p style="color:#7F1D1D;margin:6px 0 0;font-size:12px">Falha no upload pro Drive — peça pro cliente reenviar antes de prosseguir.</p>' +
      '</div>';
  }

  var linkPastaHtml = data.pastaDriveUrl
    ? '<p style="font-size:13px"><a href="' + data.pastaDriveUrl + '" style="color:#087bb5;font-weight:bold">📁 Abrir pasta com todos os documentos no Drive →</a></p>'
    : '';
  ...
  var html =
    ...
    '<div style="background:#f9f9f9;padding:28px 32px">' +
      ...
      '<ul style="font-size:14px;color:#333">' +
        docsTitular.map(function (d) { return '<li>' + d.descricao + ': <a href="' + d.url + '" style="color:#087bb5">' + d.nome + '</a></li>'; }).join('') +
      '</ul>' +
      secaoDocsConjuge +
      linkPastaHtml +
      secaoAlertaDocs +
  ...
  // 3. Envia e-mail para a imobiliária (sem anexo — os documentos são
  // links do Drive, já embutidos no html acima)
  enviarEmailBrevo_(EMAIL_DESTINO, assunto, html, { replyTo: data.email });
  ...
  // 6. Grava na aba RESERVA — colunas 19 (Link Documentos Drive) e 20
  // (Docs Faltando) novas em 78.1; se a aba já existir sem elas, roda
  // configurarColunasDriveReserva() uma vez (ver função abaixo).
  if (LOG_PLANILHA) {
    try {
      var ss  = SpreadsheetApp.openById(PLANILHA_ID);
      ...
      if (!aba) {
        ...
        aba.appendRow([
          ...
          'Link Documentos (Drive)', 'Docs Faltando'
        ]);
        aba.getRange(1, 1, 1, 20)
...
/* ─── ADICIONA AS COLUNAS 'Link Documentos (Drive)' E 'Docs Faltando'
   NA ABA RESERVA JÁ EXISTENTE — processarReserva() só escreve o
   cabeçalho quando cria a aba do zero (mesmo motivo de
   configurarColunaEmailLeads/configurarColunaMensagemLeads acima). Rode
   esta função uma única vez (▶ no editor) depois de publicar a versão
   com o upload pro Drive (78.1). */
function configurarColunasDriveReserva() {
  var ss  = SpreadsheetApp.openById(PLANILHA_ID);
  var aba = ss.getSheetByName(ABA_RESERVA);
  if (!aba) { Logger.log('Aba ' + ABA_RESERVA + ' não encontrada — será criada com as colunas novas na 1ª reserva.'); return; }
  var header = aba.getRange(1, 19, 1, 2);
  header.setValues([['Link Documentos (Drive)', 'Docs Faltando']]);
  header.setBackground('#0A1628').setFontColor('#C9A84C').setFontWeight('bold');
  aba.setColumnWidth(19, 260);
  Logger.log('✅ Colunas "Link Documentos (Drive)" e "Docs Faltando" criadas na aba ' + ABA_RESERVA + ' (colunas S e T).');
}
...
/* ─── TESTE RESERVA (78.1) — envia os 2 e-mails, o WhatsApp e grava na
   aba RESERVA com URLs de documento FAKE (não sobe nada de verdade no
   Drive, só testa o fluxo de e-mail/planilha deste projeto). Pra testar
   o upload real, use reserva.html de ponta a ponta. */
function testarReserva() {
  var dados = {
    timestamp: new Date().toLocaleString('pt-BR'),
    nome: 'TESTE Automático Reserva', nascimento: '01/01/1990',
    telefone: '(21) 99999-9999', email: 'teste@walservidor.com.br',
    cpf: '000.000.000-00', estado_civil: 'Solteiro(a)',
    empreendimento: 'Empreendimento Teste', faixa_valor: 'R$ 500.000 a R$ 700.000',
    valor_entrada: 'R$ 50.000', forma_pagamento: 'CFIAe',
    detalhes: 'Reserva de teste gerada por testarReserva().',
    pastaDriveUrl: 'https://drive.google.com/drive/folders/TESTE',
    documentosFaltando: 0,
    documentos: [
      { descricao: 'Identidade (RG/CNH)', nome: 'rg-teste.pdf', url: 'https://drive.google.com/file/d/TESTE1' },
      { descricao: 'CPF', nome: 'cpf-teste.pdf', url: 'https://drive.google.com/file/d/TESTE2' }
    ]
  };
  Logger.log('Iniciando testarReserva com dados: ' + JSON.stringify(dados));
  processarReserva(dados);
  Logger.log('✅ testarReserva concluído — confira os e-mails (comercial@walservidor.com.br e teste@walservidor.com.br) e a aba "' + ABA_RESERVA + '"');
}
